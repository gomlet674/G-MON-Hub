...
-- Tunggu game siap: gunakan event Load if game:IsLoaded then repeat task.wait() until game:IsLoaded() else game.Loaded:Wait() end print("[GMON Loader] Game loaded, PlaceId =", game.PlaceId)
...
-- Loader GUI di CoreGui atau PlayerGui local guiRoot = (typeof(CoreGui) == "Instance" and CoreGui) or playerGui
...
-- Container utama local frame = Instance.new("Frame", loaderGui) frame.Name = "Container" frame.Size = UDim2.new(0, 350, 0, 150) frame.Position = UDim2.new(0.5, -175, 0.5, -75) frame.BackgroundColor3 = Color3.fromRGB(20,20,20) frame.ZIndex = 2 frame.Active = true

Instance.new("UICorner", frame).CornerRadius = UDim.new(0,8)

-- Judul local title = Instance.new("TextLabel", frame) title.Size = UDim2.new(1,0,0,30) title.Position = UDim2.new(0,0,0,5) title.Text = "GMON Hub Key" title.Font = Enum.Font.GothamBold title.TextSize = 18 title.TextColor3 = Color3.new(1,1,1) title.BackgroundTransparency = 1

-- Input Key local keyBox = Instance.new("TextBox", frame) keyBox.Size = UDim2.new(0.9,0,0,30) keyBox.Position = UDim2.new(0.05,0,0,50) keyBox.PlaceholderText = "Enter your key..." keyBox.Font = Enum.Font.Gotham keyBox.TextColor3 = Color3.new(1,1,1) keyBox.BackgroundColor3 = Color3.fromRGB(40,40,40) Instance.new("UICorner", keyBox).CornerRadius = UDim.new(0,5)

-- Tombol Submit local submitBtn = Instance.new("TextButton", frame) submitBtn.Size = UDim2.new(0.4,0,0,30) submitBtn.Position = UDim2.new(0.05,0,1,-40) submitBtn.Text = "Submit" submitBtn.Font = Enum.Font.GothamSemibold submitBtn.TextSize = 16 submitBtn.TextColor3 = Color3.new(1,1,1) submitBtn.BackgroundColor3 = Color3.fromRGB(0,170,127) Instance.new("UICorner", submitBtn).CornerRadius = UDim.new(0,5)

-- Mapping PlaceId ke URL local GAME_SCRIPTS = { [4442272183] = "https://raw.githubusercontent.com/gomlet674/G-MON-Hub/main/main.lua", [3233893879] = "https://raw.githubusercontent.com/gomlet674/G-MON-Hub/main/main_arena.lua", [537413528]  = "https://raw.githubusercontent.com/gomlet674/G-MON-Hub/main/build.lua", }

local VALID_KEY = "GmonHub311851f3c742a8f78dce99e56992555609d23497928e9b33802e7127610c2e" local keyFile = "gmon_key.txt"

-- Fungsi load script game local function loadGameScript() local url = GAME_SCRIPTS[game.PlaceId] if not url then warn("[GMON Loader] Game tidak dikenali: " .. tostring(game.PlaceId)) showCenterNotification("Game not supported!", 2) return end showCenterNotification("Loading...") print("[GMON Loader] Loading URL: "..url) local ok, err = pcall(function() loadstring(game:HttpGet(url, true))() end) if not ok then warn("[GMON Loader] Error load script:", err) end end
...
-- Auto-load jika key tersimpan if isfile(keyFile) then local saved = readfile(keyFile) if saved == VALID_KEY then loaderGui:Destroy() loadGameScript() else print("[GMON Loader] Saved key invalid, deleting file.") delfile(keyFile) end end
