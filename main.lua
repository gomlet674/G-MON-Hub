...
-- 1) Switch (toggle) tanpa teks "Button"
local function AddSwitch(page, labelText, flag)
    local ctr = New("Frame", {
    ...
    local sw = New("TextButton", {
        Text = "",               -- sembunyikan teks default
        TextTransparency = 1,    -- benar-benar sembunyikan
...
-- 2) Dropdown
local function AddDropdown(page, labelText, list, flag)
    New("TextLabel", {
        Text = labelText,
...
-- 3) Toggle biasa
local function AddToggle(page, labelText, flag)
    local btn = New("TextButton", {
        Text = labelText,
        Size = UDim2.new(1,0,0,30),
...
-- 4) Text label statis
local function AddText(page, str)
    New("TextLabel", {
        Text = str,
        Size = UDim2.new(1,0,0,20),
        BackgroundTransparency = 1,
...
-- BACKGROUND IMAGE (di balik frame)
New("ImageLabel", {
...
-- MAIN LOGIC (AutoFarm, FarmBoss, FarmChest)
task.spawn(function()
    while true do
...
do local p = pages.Misc
    ...
    AddToggle(p,"Auto Awaken",     "AutoAwaken")
end
