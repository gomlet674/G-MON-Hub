...
-- ===== BLOX FRUIT BARU MODULE (REPLACED & FIXED) ===== -- Improved, safer AutoFarm + AutoStats + equips + attack fallback 
...
-- AttemptAttack: prefer server remote if available, else 

Local fallback damage
local function AttemptAttackBetter(target)
    ...
    -- If remote didn't exist or had no effect, attempt 

local damage as last resort (non-intrusive)
    if not ok then
        pcall(function()
